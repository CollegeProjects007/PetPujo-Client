...
export default function CurrentOrdersScreen({ route, navigation }) {
  ...
  useEffect(() => {
    currentOrders()
      .then((response) => {
        ...
        setCurrentItems(response.data.currentOrders);
        setOrderedItems(response.data.currentOrders.items);
      })
      .catch((error) => {
        console.log(error);
      });
